...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val top_10 = flipped.sortBy(_._1, ascending = false).take(10)
    val bot_10 = flipped.sortBy(_._1, ascending = true).take(10)
    
    for (dude <- top_10){
      val name = namesRdd.lookup(dude._2)(0)
      println(s"$name is a popular superhero with ${dude._1} co-appearances.")    
    }
    for (dude <- bot_10){
      val name = namesRdd.lookup(dude._2)(0)
      println(s"$name is an unpopular superhero with ${dude._1} co-appearances.")    
    }
