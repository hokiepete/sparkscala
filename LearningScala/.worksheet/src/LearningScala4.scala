...
val numlist = List.range(1,21).filter( (x: Int) => x%3 == 0);System.out.println("""numlist  : List[Int] = """ + $show(numlist ));$skip(18); 

println(numlist);$skip(57); 

println(List.range(1,21).filter( (x: Int) => x%3 == 0))}
